...
function initRain() {
  ...
  cols = Array.from({ length: n }, () => Math.floor(Math.random() * MC.height / FS));
}
...
function drawRain() {
  ...
  cols.forEach((y, i) => {
    ...
    CTX.fillStyle    = (i % 9 === 0) ? '#ffffff' : (i % 4 === 0) ? '#00ffaa' : '#00ff41';
...
document.addEventListener('mousedown', () => { curEl.style.width = '14px'; curEl.style.height = '14px'; });
document.addEventListener('mouseup',   () => { curEl.style.width = '10px'; curEl.style.height = '10px'; });
...
function playTone(freq, dur, type = 'square', vol = 0.12) {
  ...
  } catch (e) { /* AudioContext may be blocked until user interaction */ }
}

function playKeyClick() { playTone(800 + Math.random() * 400, 0.04, 'square', 0.06); }
function playBeep()     { playTone(1200, 0.1, 'square', 0.1); }
...
function playAlert()  { playTone(880, 0.15, 'sawtooth', 0.1); }

function playGlitch() {
  for (let i = 0; i < 5; i++)
    setTimeout(() => playTone(50 + Math.random() * 200, 0.05, 'sawtooth', 0.08), i * 30);
}
...
// ── 5. Game State & Constants ────────────────────────────────────
const DIFF = {
  easy: { timer: 120, maxAttempts: 8, traceSpeed: 0.4, pwLen: 4, label: 'EASY' },
...
function tEmpty(delay = 0) { return tLine('', 'emp', delay); }
...
function tCmd(cmd) {
  tLine(`<span class="psym">$</span><span class="pusr"> ${escHtml(cmd)}</span>`, 'cmd');
}
...
// ── 8. Boot Sequence ─────────────────────────────────────────────
const BOOT_LINES = [
  { t: '[ BIOS v3.9.1 ] Initializing quantum processor array...',         s: 'ok',   d: 200  },
  { t: '[ MEM ] Allocating 128TB secure enclave...',                       s: 'ok',   d: 350  },
  { t: '[ NET ] Connecting to encrypted node cluster [7731]...',           s: 'ok',   d: 500  },
  { t: '[ SEC ] Validating cryptographic certificates...',                  s: 'ok',   d: 650  },
  { t: '[ FW ] Probing firewall topology...',                               s: 'warn', d: 820  },
  { t: '[ FW ] External intrusion attempt detected — suppressing...',       s: 'warn', d: 1000 },
  { t: '[ FW ] Bypass route established via onion relay #44...',            s: 'ok',   d: 1200 },
  ...
  { t: '[ ERR ] Anomalous activity pattern — flagged by AI_CORE',           s: 'er',   d: 2200 },
...
document.getElementById('btnNew').addEventListener('click', () => {
  clearSave(); G.level = 1; startGame();
});

document.getElementById('btnCont').addEventListener('click', () => {
  const sv = loadSave();
  if (sv) { G.level = sv.level; G.diff = sv.diff || 'med'; }
  startGame();
});

document.getElementById('btnDiff').addEventListener('click', () => showScreen('sdiff'));

document.getElementById('btnSound').addEventListener('click', () => {
  soundOn = !soundOn; updateSoundBtn(); playBeep();
});
...
document.getElementById('btnRetry').addEventListener('click',   () => startLevel(G.level));
document.getElementById('btnMenuGov').addEventListener('click', () => { clearSave(); showMenu(); });
...
// ── 10. Game Start & Level Manager ───────────────────────────────
function startGame() { startLevel(G.level); }
...
function startTrace(speed) {
  G.traceInterval = setInterval(() => {
    ...
    if (G.trace >= 100) {
      clearInterval(G.traceInterval);
      gameOver('TRACE DETECTION REACHED 100%\nLOCATION COMPROMISED — SYSTEM LOCKDOWN');
    }
    if (G.trace >= 80 && G.trace < 80 + speed) {
      playAlert();
      tLine('⚠ WARNING: TRACE LEVEL CRITICAL', 'wrn');
      aiMsg('Trace detection near threshold. Recommend immediate countermeasures.');
    }
  }, 1000);
}
...
function updateTrace(val) {
  ...
  document.getElementById('tval').textContent    = Math.round(G.trace) + '% — ' + label;
}
...
async function levelComplete(msg = '') {
  ...
  showAch(`LEVEL ${G.level} CLEARED`, MISSIONS[G.level - 1].name + ' — COMPLETE');
  G.level++;
  saveGame();
...
async function gameOver(reason = '') {
  ...
  document.getElementById('govTxt').innerHTML =
    ...
    escHtml(reason) + '<br><br>AGENT IDENTITY COMPROMISED<br>SYSTEM QUARANTINE INITIATED';
...
// ══════════════════════════════════════════════════════════════════
const WORDS = {
  4: ['ECHO','NODE','BYTE','CODE','HACK','KERN','FLUX','GRID','LOCK','MASK','ROOT','SCAN','VOID','WORM','ZERO'],
  5: ['ALPHA','BRAVO','CRYPT','DELTA','ETHER','GHOST','HYDRA','INDEX','KARMA','LASER','NEXUS','OMEGA','PIXEL','QUARK','RELAY','SIGMA','TOXIN','ULTRA','VIPER','XENON'],
  6: ['BINARY','CIPHER','DAEMON','ENIGMA','FALCON','GLITCH','HUNTER','ICARUS','JUNGLE','KERNEL','MATRIX','NEURAL','ORACLE','PACKET','QUARRY','REBOOT','SOCKET','TROJAN','UPLINK','VECTOR']
};
...
async function runLevel1() {
  ...
  G.levelState = { password, maxAtt, attempts: 0, hintsUsed: 0, done: false, revealed: [] };

  setAttempts(maxAtt, 0);
  setHint(`Target: ${d.pwLen}-character code word<br>Type: <b style="color:var(--g)">crack [word]</b><br>Or: <b style="color:var(--g)">hint</b> for a clue`);
  ...
  await tEmpty();
  await tLine(`TARGET HASH: ${'*'.repeat(password.length * 3).slice(0, 32)} [TRUNCATED]`, 'wrn');
  ...
  await tLine('  hint           — request cryptographic hint (costs 1 attempt)', 'sys');
  await tLine('  scan           — analyze character frequency (costs 1 attempt)', 'sys');
  await tEmpty();

  startTimer(timerSec,
    s => { if (s === 30 || s === 15 || s === 10) { playAlert(); aiMsg('Timer approaching critical threshold.'); } },
...
function handleLevel1(cmd) {
  ...
  if (lower.startsWith('crack ')) {
    ...
    const feedback = ls.password.split('').map((ch, i) => {
      ...
      if (ls.password.includes(guess[i] || '')) return { ch: guess[i], state: 'hit' };
      return { ch: guess[i] || '_', state: 'mis' };
    });
    showScanResult(feedback.map(f => f.ch), feedback.map(f => f.state));

    const correct = ls.password.split('').filter((ch, i) => guess[i] === ch).length;
    tLine(correct > 0 ? `${correct}/${ls.password.length} CHARS IN CORRECT POSITION` : 'NO POSITIONAL MATCHES',
          correct > 0 ? 'wrn' : 'err');
    ...
    ls.attempts++; ls.hintsUsed++;
    setAttempts(ls.maxAtt, ls.attempts);

    const unrevealed = ls.password.split('').map((_, i) => i).filter(i => !ls.revealed.includes(i));
    if (unrevealed.length) {
      const idx = unrevealed[Math.floor(Math.random() * unrevealed.length)];
      ls.revealed.push(idx);
      const hint = ls.password.split('').map((ch, i) => ls.revealed.includes(i) ? ch : '_').join(' ');
      tLine(`HINT: Character at position ${idx + 1} is "${ls.password[idx]}"`, 'inf');
      tLine(`KNOWN: ${hint}`, 'inf');
      setHint(`Known: <b style="color:var(--g);letter-spacing:.2em">${hint}</b>`);
    }
    ...
    const freq   = unique.map(ch => `${ch}:${ls.password.split(ch).length - 1}`).join('  ');
    tLine(`CHAR FREQUENCY ANALYSIS: ${freq}`, 'inf');
    tLine(`UNIQUE CHARS: ${unique.length}  TOTAL: ${ls.password.length}`, 'inf');

  } else {
    tLine(`UNKNOWN COMMAND: "${escHtml(cmd)}" — try: crack [word], hint, scan`, 'err');
  }
}
...
// ══════════════════════════════════════════════════════════════════
const CMD_LEVELS = [
  { cmd: 'ls -la',                                      hint: 'List directory contents with permissions flag',          resp: 'total 42\ndrwxr-x--- 3 root matrix 4096 ...\n-rw------- 1 root root  1024 shadow.db\n-rw-r--r-- 1 root root   512 access.log' },
  { cmd: 'sudo su -',                                   hint: 'Escalate to superuser mode',                             resp: '[sudo] password for agent: ****\nroot@matrix:~#' },
  { cmd: 'cat shadow.db',                               hint: 'Read the shadow database file',                          resp: 'matrix_admin:$6$xyz$ENCRYPTED_HASH_REDACTED\ncitizen_7731:$6$abc$ENCRYPTED_HASH_REDACTED' },
  { cmd: 'decrypt --key matrix shadow.db',              hint: 'Decrypt using "matrix" as the key with the decrypt tool', resp: 'Decrypting with key [matrix]...\nFOUND CREDENTIALS: admin:Zx9#m@tr1x\nACCESS LEVEL: ROOT' },
  { cmd: 'connect --host core.matrix.net --user admin', hint: 'Connect to core.matrix.net as admin',                   resp: 'Establishing encrypted tunnel...\nAuthentication success.\nWelcome, admin. Matrix core online.' }
];
...
async function runLevel2() {
  ...
  await tLine('└─────────────────────────────────────┘', 'out');
  await tEmpty();
  aiMsg('System access incomplete. Terminal privileges insufficient. Manual escalation required.');
  await tEmpty();
  await tLine('You must navigate the matrix file system and extract credentials.', 'out');
  await tLine(`Complete ${CMD_LEVELS.length} commands to infiltrate the core.`, 'out');
  await tEmpty();

  startTimer(d.timer + 30,
    s => { if (s <= 20) playAlert(); },
...
function showNextCmd() {
  ...
  document.getElementById('tpr').textContent = ls.cmdIdx >= 1 ? 'root@matrix:~#' : 'agent@matrix:~$';
}
...
function handleLevel2(cmd) {
  ...
  if (cmd.trim().toLowerCase() === step.cmd.toLowerCase()) {
    ...
    if (ls.cmdIdx >= CMD_LEVELS.length) {
      ...
      stopTimer();
      clearInterval(G.traceInterval);
      setTimeout(() => levelComplete('ALL COMMANDS EXECUTED — MATRIX CORE BREACHED'), 400);
    } else {
      tEmpty();
      showNextCmd();
    }
...
// ══════════════════════════════════════════════════════════════════
const CIPHERS = [
  { q: 'Decode Caesar (shift 3): PDWULA',   a: 'MATRIX', hint: 'Shift each letter back by 3' },
  { q: 'Decode binary: 01000111 01001111',  a: 'GO',     hint: 'ASCII: A=65, B=66...' },
  { q: 'Hex to text: 48 41 43 4B',          a: 'HACK',   hint: 'Convert each hex pair to ASCII' },
  { q: 'Caesar (shift 13, ROT13): ZNGEVK',  a: 'MATRIX', hint: 'ROT13: rotate each letter 13 places' },
  { q: 'Decode: Each letter + 1 in alphabet: RZMS', a: 'SALT',   hint: 'R→S, Z→A? No — shift back by 1' },
  { q: 'Reverse this string: ESACPE',       a: 'ESCAPE', hint: 'Read it backwards' },
  { q: 'Decode Caesar (shift 1): IBDL',     a: 'HACK',   hint: 'Shift back 1: H-A-C-K' },
  { q: 'ASCII decimal: 82 79 79 84',        a: 'ROOT',   hint: 'R=82, O=79, O=79, T=84' },
  { q: 'Decode Caesar (shift 5): YMFYWD',   a: 'THREAT', hint: 'Shift back 5 positions in alphabet' },
...
async function runLevel3() {
  ...
  updateTrace(0);
  setHint('Solve cipher challenges<br>to reduce trace level.<br>Type: <b style="color:var(--g)">decode [answer]</b>');

  await tLine('┌─────────────────────────────────────┐', 'inf');
  await tLine('│   LEVEL 3 — TRACE COUNTERMEASURES   │', 'inf');
  await tLine('└─────────────────────────────────────┘', 'out');
  await tEmpty();
  aiMsg('AI_CORE trace algorithms activated. Solve cipher challenges to mask signal.');
  await tEmpty();
  await tLine('MISSION: Trace detection is rising. Each solved cipher reduces trace by 25%.', 'out');
...
function handleLevel3(cmd) {
  ...
  if (answer === ch.a.toUpperCase()) {
    ...
    if (ls.idx >= ls.challenges.length) {
      ...
      tLine(`TRACE REDUCED — CHALLENGE ${ls.idx + 1}/${ls.challenges.length}:`, 'inf');
      tLine(next.q, 'hl');
      if (G.diff === 'easy') tLine(`HINT: ${next.hint}`, 'sys');
      setHint(`Challenge ${ls.idx + 1}/${ls.challenges.length}<br>${next.hint}`);
    }
  } else {
    playFail();
...
const FW_COLORS = ['#00ff41', '#00d4ff', '#ff8c00', '#cc00ff', '#ff0040', '#00ffcc'];
...
async function runLevel4() {
  ...
  setHint('Watch the node sequence.<br>Click nodes in order.<br>Wrong = +alert level');

  await tLine('┌─────────────────────────────────────┐', 'inf');
  await tLine('│    LEVEL 4 — FIREWALL BYPASS         │', 'inf');
  await tLine('└─────────────────────────────────────┘', 'out');
  await tEmpty();
  aiMsg('Firewall topology detected. Bypass requires correct node activation sequence.');
  await tEmpty();
  await tLine('MISSION: Memorize and replicate the node activation sequence.', 'out');
  await tLine('Wrong node selection increases alert level. 3 errors = lockout.', 'wrn');
  await tEmpty();

  buildFirewallNodes();

  startTimer(d.timer + 40,
    null,
...
async function nextFirewallRound() {
  ...
  tLine(`ROUND ${ls.round} — Observe sequence (${ls.sequence.length} nodes)...`, 'inf');
...
async function handleLevel4Click(nodeIdx) {
  ...
  if (nodeIdx !== expected) {
    ...
    if (btn) { btn.classList.add('err'); setTimeout(() => btn.classList.remove('err'), 400); }
    playFail();
    ls.alertLevel++;
    tLine(`WRONG NODE — ALERT LEVEL: ${ls.alertLevel}/3`, 'err');
    updateTrace(G.trace + 15);
    aiMsg(`Incorrect node sequence. Security alert raised to level ${ls.alertLevel}.`);
    ls.playerSeq = [];
    updateSeqDisplay([]);
...
// ══════════════════════════════════════════════════════════════════
const ESCAPE_CMDS = [
  { cmd: 'arm escape.sequence',      hint: 'Initialize the escape protocol',       resp: 'ESCAPE SEQUENCE ARMED\nCountdown initiated. Commencing system extraction...' },
  { cmd: 'override --force lockdown', hint: 'Force override the lockdown protocol', resp: 'LOCKDOWN OVERRIDE: ACCEPTED\nSecurity protocols disabling...\nBackdoor tunnel established.' },
  { cmd: 'eject matrix.core',         hint: 'Eject from the matrix core',           resp: 'EJECTION SEQUENCE: INITIATED\nDisconnecting from all nodes...\nFreedom protocol engaged.' }
];
...
async function runLevel5() {
  ...
  await tEmpty();
  await tLine(`ENTER ${ESCAPE_CMDS.length} COMMANDS TO ESCAPE THE MATRIX:`, 'wrn');
  await tEmpty();
  showEscapeCmd();
...
function showEscapeCmd() {
  ...
  tLine(`── COMMAND ${ls.cmdIdx + 1}/${ESCAPE_CMDS.length}: ${step.hint}`, 'inf');
  if (G.diff !== 'hard') tLine(`COMMAND: ${step.cmd}`, 'wrn');
  setHint(`Step ${ls.cmdIdx + 1}/${ESCAPE_CMDS.length}<br>${step.hint}<br><br>${G.diff !== 'hard' ? step.cmd : '???'}`);
}
  ...
  if (cmd.trim().toLowerCase() === step.cmd.toLowerCase()) {
    ...
    if (ls.cmdIdx >= ESCAPE_CMDS.length) {
      ...
      setTimeout(async () => {
        ...
        await sleep(600);
        levelComplete('THE MATRIX HAS BEEN ESCAPED\nAGENT SUCCESSFULLY EXTRACTED');
      }, 500);
    } else {
      tEmpty();
      showEscapeCmd();
